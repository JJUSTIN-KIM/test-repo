...
int p_price = 0, p_count = 0, point = 0, membership = 0, delivery_price = 0, multi_price = 0, multi_exp_price = 0, total_price = 0;
...
<link rel="stylesheet" type="text/css" href="css/order.css" />
	<div class="contents_area">
			...
			<table class="table_order_list">
				...
				<tbody>
					...
					<tr>
						...
						결제금액 : <strong><%=multi_exp_price %>원 + 배송료 <%=delivery_price %>원 - 할인 <%=point + membership %>원 = 총 <%=total_price %>원</strong>
						<% if (isLogin) { %> (배송 후 <%=multi_exp_price / 10 %>원 적립) <% } %>
						</td>
					</tr>				
				</tbody>
			</table>
		...
		<ul class="order_number_info">
			...
			<% if (isLogin) { %>
			<li>귀하의 제품 구입에 따른 적립금 <strong><%=multi_exp_price / 10 %></strong>포인트 구매결정 완료 후 바로 적립됩니다.</li>
			<% } %>
