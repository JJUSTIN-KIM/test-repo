...
function countChange(changeType, idx) {
	var frm = (document.getElementsByName('frm_cart'))[idx];
	var amount = parseInt(frm.qtyCart.value);
	if (typeof frm.min_add_amount == 'undefined') {
		var min_add_amount = 1;
	} else {
		var min_add_amount = parseInt(frm.min_add_amount.value);
	}
	if (changeType == 1) {
	...
	frm.qtyCart.value = amount;
}

function send_cart(idx, ctype) {
	var frm = (document.getElementsByName('frm_cart'))[idx];
	var obj = frm.qtyCart.value;
	if (ctype == 'del') {
		if (confirm('삭제하시겠습니까?')) {
			frm.ctype.value = 'del';
		...
		frm.qtyCart.value = frm.qtyCart.value.replace(/-/g, '');
		if (isNaN(frm.qtyCart.value) || frm.qtyCart.value == '0') {
			alert('수량을 정확히 입력하세요.');
			return;
		} else {
			frm.ctype.value = 'up';
			frm.action = 'order_cart_proc.jsp?selectamount=' + obj;
			frm.submit();
		}
	}
}
...
<div class="contents_area">
	...
	<div class="section_cart_list">
		...
		<tr>
			...
			<td>
				<div class="cart_qty">
					<a href="javascript:countChange(0, <%=count - 1 %>);"><img src="images/btn_down.gif" /></a>
					<input type="text" name="qtyCart" class="qtyCart" id="qtyCart" value="<%=selectamount %>" readonly="readonly" />
					<a href="javascript:countChange(1, <%=count - 1 %>);"><img src="images/btn_up.gif" /></a>
				</div>
				<div class="cart_qty_modify">
					<a href="javascript:send_cart(<%=count - 1 %>, 'up')"><img src="images/btn_qty.png" /></a>
				</div>
			</td>
			<td><%=p_price %>원</td>
			...
			<td>
				<a href="javascript:send_cart(<%=count - 1 %>, 'del')"><img src="images/btn_remove.png" /></a>
			</td>
		</tr>
		</form>
		<%
