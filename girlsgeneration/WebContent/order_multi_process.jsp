...
String[] img = new String[cartCount];
for (int k = 0 ; k < cartCount ; k++) {
	img[k] = request.getParameter("img" + countArr[k]);
}

String[] title = new String[cartCount];
for (int l = 0 ; l < cartCount ; l++) {
	title[l] = request.getParameter("title" + countArr[l]);
}
...
int[] multi_price = new int[cartCount];
for (int r = 0 ; r < cartCount ; r++) {
	multi_price[r] = Integer.parseInt(request.getParameter("multi_price" + countArr[r]));
}

int[] reserve = new int[cartCount];
for (int s = 0 ; s < cartCount ; s++) {
	reserve[s] = Integer.parseInt(request.getParameter("reserve" + countArr[s]));
}

int[] multi_reserve = new int[cartCount];
for (int t = 0 ; t < cartCount ; t++) {
	multi_reserve[t] = Integer.parseInt(request.getParameter("multi_reserve" + countArr[t]));
}

int count = 0, delivery_price = 0, discountRate = 0, discount_price = 0, total_price = 0, total_reserve = 0, total_exp_price = 0;
int point = 0, qty_point = 0;

for (int u = 0 ; u < cartCount ; u++) {
	total_exp_price += multi_price[u];
	total_reserve = total_exp_price / 10;
}

if (total_exp_price >= 50000) {
	delivery_price = 0;
} else {
	delivery_price = 2500;
}
...
%>
	...
	<form name="frm_order_process" action="order_multi_proc.jsp" method="post" onsubmit="return chkOrderForm(this);">
	...
	<input type="hidden" name="isChk" />
		...
		<div class="section_order_list">
			...
			<table class="table_order_list">
				...
				<tbody>
				<% for (int a = 0 ; a < cartCount ; a++) { count++; %>
					<input type="hidden" name="p_id<%=count %>" value="<%=p_id[a] %>" />
					<input type="hidden" name="img<%=count %>" value="<%=img[a] %>" />
					<input type="hidden" name="title<%=count %>" value="<%=title[a] %>" />
					<input type="hidden" name="height<%=count %>" value="<%=height[a] %>" />
					<input type="hidden" name="selectsize<%=count %>" value="<%=selectsize[a] %>" />
					<input type="hidden" name="selectcolor<%=count %>" value="<%=selectcolor[a] %>" />
					<input type="hidden" name="selectamount<%=count %>" value="<%=selectamount[a] %>" />
					<input type="hidden" name="p_price<%=count %>" value="<%=p_price[a] %>" />
					<input type="hidden" name="multi_price<%=count %>" value="<%=multi_price[a] %>" />
					<input type="hidden" name="reserve<%=count %>" value="<%=reserve[a] %>" />
					<input type="hidden" name="multi_reserve<%=count %>" value="<%=multi_reserve[a] %>" />
					<tr>
						<td class="td_product_image" width="5%">
							<a href="product_detail.jsp?p_id=<%=p_id[a] %>"><img src="<%=img[a] %>" /></a>
						</td>
						<td class="td_product_info" width="*">
							<div class="box_product"><%=title[a] %></div>
							<div class="box_product_height"><%=height[a] %> cm</div>
						</td>
						<td class="td_price" width="10%">
							<div class="box_product_price"><%=p_price[a] %> 원</div>
						</td>
						<td class="td_product" width="15%">
							<div class="box_product_ea"><%=selectamount[a] %> ea</div>
						</td>
						<td class="td_price" width="15%">
							<div class="box_product_price"><%=multi_price[a] %> 원</div>
						</td>
						<td class="td_reserve" width="15%">
							<div class="box_product_reserve"><%=multi_reserve[a] %></div>
						</td>
					</tr>
					<tr>
						<td colspan="6">
							<div class="option">
								option : color - <%=selectcolor[a] %>, size : <%=selectsize[a] %>
							</div>
						</td>
					</tr>
<%
...
%>			
			...
			if (isLogin && rs.next()) {
				...
				discount_price = total_exp_price * discountRate / 100;
			}
			
			total_price = total_exp_price + delivery_price - discount_price;
			...
			<div class="membership_order">
				...
				<div class="membership_discount_info"><%=userName %>님의 할인 예정 금액은 <strong><input type="text" name="qty_membership" id="qty_membership" value="<%=discount_price %>" readonly="readonly" />원</strong>입니다.</div>
			</div>
			<div class="ask_point">
				<span>보유 적립금 사용</span>
				<span>
		...
		<div class="order_basic">
			...
			<table class="table_order_basic">
				...
				<tr>
					<th>연락처</th>
					<td>
						<input type="text" name="order_phone1" class="order_phone" value="<%=phone1 %>" onKeyUp="onlyNumber(this); removeChar(event);" maxlength="3" /> - 
						<input type="text" name="order_phone2" class="order_phone" value="<%=phone2 %>" onKeyUp="onlyNumber(this); removeChar(event);" maxlength="4" /> -
						<input type="text" name="order_phone3" class="order_phone" value="<%=phone3 %>" onKeyUp="onlyNumber(this); removeChar(event);" maxlength="4" />
					</td>
				</tr>
			</table>
		</div>
		...
		<div class="order_discount">
			...
			<table class="table_order_discount">
				...
				<tr>
				<td><input type="text" name="p_price" class="price" value=<%=total_exp_price %> readonly="readonly" />원</td>
				<td>+</td>
		...
		<div class="order_paymethod">
			...
			<table class="table_order_paymethod">
				...
				<td>
					<input type="button" value="본인 확인" name="btn_confirm" onclick="cardConfirm();" />&nbsp;&nbsp;&nbsp;&nbsp;
		...
		<div class="order_confirm">
			...
			<table class="table_order_final">
				...
				<td width="*"><strong id="price"><input type="text" name="finalPrice" class="finalPrice" id="finalPrice2" value="<%=total_price %>" readonly="readonly" /></strong><strong id="won">원</strong> (적립예정 : <%=total_reserve %>원)</td>
				</tr>
			</table>
		<div class="order_btn">
